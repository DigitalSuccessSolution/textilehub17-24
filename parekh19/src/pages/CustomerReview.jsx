        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
          {reviews.map((review, idx) => (
            ...
            >
              {/* Quote Icon */}
              <div className="mb-5">
                <Quote size={30} color="#244C73" className="rotate-180" fill="rgba(95,111,94,0.15)" />
              </div>

              {/* Text */}
              <p className="leading-relaxed text-[13.5px] mb-6 flex-grow line-clamp-4 text-[#393E46] font-medium">
                "{review.text}"
              </p>

              {/* Footer */}
              <div className="flex items-center justify-between mt-auto pt-5 border-t border-[#E8E5DC]/60">
                <div className="flex items-center gap-3">
                  <img
                    src={review.image}
                    alt={review.name}
                    className="w-11 h-11 rounded-full object-cover shadow-sm border border-[#E8E5DC]"
                    style={{ border: '2px solid rgba(95,111,94,0.25)' }}
                  />
                  <div>
                    <h4 className="font-bold text-[14px] text-[#152E47] uppercase tracking-wide">{review.name}</h4>
                    <p className="text-[10px] mt-0.5 text-[#6B7280] font-semibold">{review.role}</p>
                  </div>
                </div>
                <ArrowRight size={18} color="#244C73" className="group-hover:translate-x-1 transition-transform duration-200" />
              </div>
            </motion.div>
          ))}
        </div>
