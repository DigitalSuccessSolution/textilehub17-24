  ...
  return (
    <div className="pb-16 w-full min-h-screen bg-[#F6F3EB]">
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-8">
        ...
        <div className="space-y-4 mb-12">
          {faqs.map((faq, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-white rounded-xl border border-[#C5A880]/15 overflow-hidden shadow-sm hover:shadow-md hover:border-[#C5A880]/30 transition-all duration-300"
              >
                {/* Question Row */}
                <button
                  onClick={() => toggleAccordion(index)}
                  className="w-full px-6 py-5 flex justify-between items-center text-left text-[#0C0C0C] font-semibold font-playfair uppercase tracking-wider text-xs md:text-sm cursor-pointer hover:text-[#C5A880] transition-colors duration-250 bg-white"
                >
        ...
        {/* Support Section */}
        <div className="bg-[#0C0C0C] border border-[#C5A880]/20 rounded-2xl p-8 text-center text-white shadow-xl relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-[#121212] to-[#0C0C0C] z-0" />
          <div className="relative z-10 max-w-xl mx-auto">
            <HelpCircle size={36} className="text-[#C5A880] mx-auto mb-4" />
            <h3 className="font-playfair text-xl md:text-2xl text-[#F3ECE0] uppercase tracking-wide font-bold mb-2">
              Still Have Questions?
            </h3>
            <p className="text-gray-400 text-xs md:text-sm font-outfit leading-relaxed mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <a
                ...
                className="flex flex-col items-center p-4 rounded-xl bg-white/5 border border-white/10 hover:border-[#C5A880]/40 transition-colors duration-300"
              >
                <Mail size={18} className="text-[#C5A880] mb-2" />
                <span className="text-[10px] uppercase font-bold tracking-widest text-[#F3ECE0]">Email Us</span>
                <span className="text-[9px] text-gray-400 mt-1 truncate w-full">support@varaweaves.com</span>
              </a>
              <a
                href="tel:+916353778329"
                className="flex flex-col items-center p-4 rounded-xl bg-white/5 border border-white/10 hover:border-[#C5A880]/40 transition-colors duration-300"
              >
                <Phone size={18} className="text-[#C5A880] mb-2" />
                <span className="text-[10px] uppercase font-bold tracking-widest text-[#F3ECE0]">Call Us</span>
                <span className="text-[9px] text-gray-400 mt-1">+91 63537 78329</span>
              </a>
              <Link
                to="/trade-enquiry"
                className="flex flex-col items-center p-4 rounded-xl bg-white/5 border border-white/10 hover:border-[#C5A880]/40 transition-colors duration-300"
              >
                <MessageSquare size={18} className="text-[#C5A880] mb-2" />
                <span className="text-[10px] uppercase font-bold tracking-widest text-[#F3ECE0]">Enquire Now</span>
                <span className="text-[9px] text-gray-400 mt-1">Submit Bulk Query</span>
              </Link>
            </div>
          </div>
        </div>
